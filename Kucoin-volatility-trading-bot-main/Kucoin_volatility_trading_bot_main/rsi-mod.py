...
if USE_MOST_VOLUME_COINS == True:
    ...
    TICKERS = 'tickerlists/tickers.txt'
    ...
    for pair in pairs:
        ...
        for indicator in OSC_INDICATORS:
            try:
                ...
                print(f"Indicator {indicator} failes for {pair}. Ignoring it for now!")
        ...
        if (RSI >= RSI_MIN and RSI <= RSI_MAX) and (RSI_DIFF >= RSI_BUY):
            if (STOCH_DIFF >= STOCH_BUY) and (STOCH_K >= STOCH_MIN and STOCH_K <= STOCH_MAX) and (
                    STOCH_D >= STOCH_MIN and STOCH_D <= STOCH_MAX):
...
def do_work():
    ...
    while True:
        try:
            ...
            print(
                f'Signals RSI: {len(signal_coins)} coins with Buy Signals. Waiting {TIME_TO_WAIT} minutes for next analysis.')
